...
func capitalize( _ str: String ) -> String{
    ...
    for i in 1..<str.count{
        ...
        if prevChar == " " || hasSpecialCharacter(char: prevChar) == true{
...
func hasSpecialCharacter( char: Character ) -> Bool{
    let str = String( char )
    let pattern = "[^\\w]"
    let regex   = try? NSRegularExpression(pattern: pattern, options: .caseInsensitive)
    var result  = regex?.stringByReplacingMatches(in: str, options: [], range: NSMakeRange(0, str.count), withTemplate: "found")
...
print( "capitalize(\"¡feliz cumpleaños!\") expects \"¡Feliz Cumpleaños!\" - \( ( capitalize("¡feliz cumpleaños!") == "¡Feliz Cumpleaños!" ) ? "PASS" : "FAIL" )" )
